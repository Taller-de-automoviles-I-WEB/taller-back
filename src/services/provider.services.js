...
const createProviderService = async (body) => {
  ...
  try {
    ...
    newProvider.password = await passwordHandler.hashPassword(
      newProvider.password
    );
    const createProvider = await db.Provider.create(newProvider, {
      transaction: t,
    });
    await t.commit();
...
const updateProviderService = async (id, updatedData) => {
  ...
  try {
    ...
    if (updatedData.password) {
      updatedData.password = await passwordHandler.hashPassword(
...
module.exports = {
  ...
  getProviderByEmailService,
  getProviderByIdService,
  updateProviderService,
  deleteProviderByIdService,
  deleteAllProvidersService,
};
